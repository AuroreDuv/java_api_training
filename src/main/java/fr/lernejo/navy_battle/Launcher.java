    ...
    public void displayGrid(GameGrid gameGrid) {
        ...
        for(int i = 0; i < 10; i++) {
            for(int j = 0; j < 10; j++) {
                ...
                else if (elmt.get_slug().equals("miss")) { displayMiss(gameGrid, i, j); }
                else if (elmt.get_slug().equals("hit")) { displayHit(gameGrid, i, j); }
                else { System.out.print(elmt.get_slug().toUpperCase().charAt(0) + "  |  "); }
            }
            System.out.print("\n");
        }
    }
